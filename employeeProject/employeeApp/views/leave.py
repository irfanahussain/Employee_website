...
from employeeApp.forms import LeaveTypeForm, LeaveRequestForm, LeaveDecisionForm, LeaveBalanceForm
...
def leave_balance_list(request):
    balances = LeaveBalance.objects.select_related('employee', 'leave_type').order_by(
        '-year', 'employee__full_name', 'leave_type__name'
    )
    year = request.GET.get('year')
    if year:
        balances = balances.filter(year=year)
    years = LeaveBalance.objects.values_list('year', flat=True).distinct().order_by('-year')
    return render(request, 'employeeApp/leave_balance_list.html', {
        'balances': balances, 'years': years, 'selected_year': year,
    })


@admin_hr_required
def leave_balance_form_view(request, pk=None):
    balance = get_object_or_404(LeaveBalance, pk=pk) if pk else None
    if request.method == 'POST':
        form = LeaveBalanceForm(request.POST, instance=balance)
        if form.is_valid():
            form.save()
            messages.success(request, f"Leave balance {'updated' if pk else 'set'} successfully.")
            return redirect('employeeApp:leave_balance_list')
    else:
        form = LeaveBalanceForm(instance=balance)
    return render(request, 'employeeApp/leave_balance_form.html', {'form': form, 'balance': balance})
...
def leave_balance_delete(request, pk):
    ...
    balance.delete()
    messages.success(request, "Leave balance removed.")
    return redirect('employeeApp:leave_balance_list')
...
def my_leave_requests(request):
    ...
    return render(request, 'employeeApp/my_leave_requests.html', {
        'page_obj': page_obj,
        'leave_form': leave_form, 'balance_map_json': json.dumps(balance_map),
    })
        ...
        if form.is_valid():
            ...
            if errors:
                ...
                if lop_days:
                    ...
                    messages.success(request, "Leave successfully applied! Your leave has been submitted.")
